...
void printArrayOfStrings(char** arr, long int size) {
    printf("size = %ld\n", size);
    // if (size > 10)
    //     size = 10;
...
void check_if_word_is_connected_to_previous(char** solution, int solution_size, char* word) {
    ...
    if (solution_size > 0) {
        ...
        if (word[0] != last_character_of_last_word) {
            printf("last char of previous word = %c, first char of this word = %c\n", last_character_of_last_word, word[0]);
...
void check_if_word_has_consecutive_same_side_letters(char** invalid_sequences, size_t total_invalid_sequences, char* word) {
    printf("check_if_word_has_consecutive_same_side_letters2\n");
...
void check_if_board_is_valid(char** board, int num_sides, bool** letters_on_board, char*** invalid_sequences, size_t* total_invalid_sequences) {
    ...
    for (int i = 0; i < num_sides; i++) {
        ...
        *invalid_sequences = (char**)realloc(*invalid_sequences, *total_invalid_sequences * sizeof(char*));
        ...
        printArrayOfStrings(*invalid_sequences, *total_invalid_sequences);
    }
    *letters_on_board = seen;
}

void printLetters(bool* letters, char* name) {
    printf("%s:\n", name);
    for (int i = 0; i < 26; i++) {
        if (letters[i])
            printf("%c,", (char)(i+97));
...
bool is_game_solved(bool* letters_on_board, bool* letters_in_solution) {
    ...
    printLetters(letters_in_solution, "letters in solution");
    for (int i = 0; i < 26; i++) {
        // letter present on board but not in solution
        if (letters_on_board[i] && !letters_in_solution[i]) {
            printf("character %c on board but not in solution\n", (char)(i+97));
            return false;
        }
    }
    return true;    
...
void play_game(char** board, long int num_sides, char** dict, long int dict_size) {
    ...
    printLetters(letters_on_board, "letters on board");
